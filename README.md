...
- Call notes, pricing, and documents remain browser-local. When configured, the hosted bridge stores public assessment submissions and verified GHL bookings; the protected inbox imports prepared packets on opening the workspace. It does not upload local edits. No AI API, analytics, or automated email/SMS is used. The unconfigured preview does not send assessment answers.
