...
export function ServicePage({ service }: { service: Service }) {
  return <>
    <article className="ww-search-page">
      ...
      <div className="ww-search-columns"><section><p className="ww-eyebrow">Ideal customer</p><h2>This is a fit when…</h2><ul>{service.ideal.map(x => <li key={x}>{x}</li>)}</ul></section><section><p className="ww-eyebrow">Process</p><h2>How the work moves</h2><ol>{service.process.map((x) => <li key={x}>{x}</li>)}</ol></section></div>
      <section className="ww-deliverables"><p className="ww-eyebrow">Deliverables</p><h2>What you receive</h2><div>{service.deliverables.map((x) => <article key={x}><h3>{x}</h3></article>)}</div></section>
...
export function GuidePage({ guide }: { guide: Guide }) {
  return <>
    <article className="ww-search-page ww-guide-page">
      ...
      {guide.sections.map((s) => <section className="ww-guide-section" key={s.title}><div><h2>{s.title}</h2><p>{s.body}</p>{s.points ? <ul>{s.points.map(x => <li key={x}>{x}</li>)}</ul> : null}</div></section>)}
